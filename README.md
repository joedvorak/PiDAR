...
7. Use the real time clock (RTC) embedded in the Shield/HAT to allow the Raspberry Pi to keep time even when disconnected from the Internet. After inserting a battery in the Shield follow the [directions from Adafruit](https://learn.adafruit.com/adding-a-real-time-clock-to-raspberry-pi?view=all)
